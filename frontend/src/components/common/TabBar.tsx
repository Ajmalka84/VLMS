...
export const TabBar = React.memo<TabBarProps>(({
  ...
  return (
    <nav
      ...
      className={`flex items-center gap-1.5 p-1.5 rounded-2xl bg-slate-900/80 border border-slate-800/80 overflow-x-auto scrollbar-none select-none ${className}`}
    >
      {tabs.map((tab) => {
        const Icon = tab.icon;
        ...
        return (
          ...
          >
            {Icon && <Icon className={`w-4 h-4 shrink-0 ${isActive ? 'text-slate-950' : 'text-slate-400'}`} />}
            <span>{tab.label}</span>
            {tab.badge !== undefined && (
              <span
