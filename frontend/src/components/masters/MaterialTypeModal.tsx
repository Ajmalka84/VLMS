import React, { useState, useEffect } from 'react';
import { AlertTriangle } from 'lucide-react';
import { Modal, Input, Button } from '../common';
import { MaterialType, createMaterialTypeApi, updateMaterialTypeApi } from '../../api/masterData';
import { useMasterCache } from '../../context/MasterCacheContext';
import { useToast } from '../../context/ToastContext';

interface MaterialTypeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: (material: MaterialType) => void;
  editItem?: MaterialType | null;
}

export const MaterialTypeModal: React.FC<MaterialTypeModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  editItem,
}) => {
  const toast = useToast();
  const { refreshMasterData } = useMasterCache();
  const [mtypeForm, setMtypeForm] = useState({ name: '' });
  const [formError, setFormError] = useState<string | null>(null);
  const [submitting, setSubmitting] = useState(false);

  useEffect(() => {
    if (isOpen) {
      if (editItem) {
        setMtypeForm({ name: editItem.name || '' });
      } else {
        setMtypeForm({ name: '' });
      }
      setFormError(null);
    }
  }, [isOpen, editItem]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!mtypeForm.name.trim()) {
      setFormError('Material Name is required');
      return;
    }

    try {
      setSubmitting(true);
      setFormError(null);
      let savedMaterial: MaterialType;
      if (editItem) {
        savedMaterial = await updateMaterialTypeApi(editItem.id, mtypeForm);
        toast.success('Material specification updated');
      } else {
        savedMaterial = await createMaterialTypeApi(mtypeForm);
        toast.success('Material specification added');
      }
      await refreshMasterData(true);
      onSuccess?.(savedMaterial);
      onClose();
    } catch (err: any) {
      setFormError(err.message || 'Failed to save material specification.');
    } finally {
      setSubmitting(false);
    }
  };

  return (
    <Modal
      isOpen={isOpen}
      onClose={() => {
        onClose();
        setFormError(null);
      }}
      title={editItem ? 'Edit Material Specification' : 'Add Material Specification'}
      maxWidth="sm"
    >
      {formError && (
        <div className="p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs font-semibold flex items-center gap-2.5 mb-4 animate-fade-in">
          <AlertTriangle className="w-4 h-4 text-rose-500 shrink-0" />
          <span className="leading-snug">{formError}</span>
        </div>
      )}

      <form onSubmit={handleSubmit} className="space-y-4">
        <Input
          label="Material Name"
          required
          placeholder="e.g. Aggregates 20mm, Sand, Rubble, Quarry Dust"
          value={mtypeForm.name}
          onChange={(e) => {
            setMtypeForm({ name: e.target.value });
            setFormError(null);
          }}
        />

        <div className="flex justify-end gap-2 pt-2 border-t border-subtle">
          <Button
            type="button"
            variant="ghost"
            size="sm"
            onClick={() => {
              onClose();
              setFormError(null);
            }}
          >
            Cancel
          </Button>
          <Button
            type="submit"
            variant="primary"
            size="sm"
            loading={submitting}
          >
            Save Material
          </Button>
        </div>
      </form>
    </Modal>
  );
};
